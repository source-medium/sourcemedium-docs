...
- **Thread follow-ups in Slack**: Continue analysis in the same thread. In channels, mention the bot with `@SourceMedium`. Start a new thread to reset context.
- **Multi-store thread scoping**: For multi-store tenants, each thread is scoped to one store after confirmation.
...
<CardGroup cols={2}>
  ...
  <Card title="Cross-Session Context" icon="brain">
    **Status: Planned**
    ...
    The AI Analyst will remember insights from previous conversations, building context over time to provide more relevant answers. Today, context is limited to the same Slack thread.
  </Card>
...
| Multi-store thread scoping | **Available (multi-store tenants only)** | Slack |
| Conversation follow-ups | **Available (Slack threads)** | Slack |
