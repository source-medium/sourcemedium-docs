...
```mermaid
flowchart LR
    A[Your Data Sources] --> B[SourceMedium Pipeline] --> C[(BigQuery)] --> D[Looker Studio & AI Analyst]
```

Your data flows from connected platforms (Shopify, Google Ads, Meta, GA4, etc.) through the SourceMedium pipeline into your BigQuery warehouse. The AI Analyst and Looker Studio dashboards both query the same underlying data — they're just different interfaces.
