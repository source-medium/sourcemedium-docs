...
### Order Attribution Audit (`fct_order_attribution_audit`)

This is the customer-facing row-level audit companion to the MTA models. Use it when you need to inspect how a specific order's attribution evidence was evaluated before you move up to journey- or report-level analysis.
...
- Start in `fct_order_attribution_audit` when a specific order looks wrong.
- Use `sm_store_id` + `order_id` to isolate the order and inspect the winning traffic row.
- Then move to `obt_purchase_journeys_with_mta_models` if you need session-level or model-level context for that same order, joining audit `order_id` to MTA `purchase_order_id` alongside `sm_store_id`.

<Info>
`mta_first_touch` and `mta_last_touch` can appear inside `fct_order_attribution_audit`, but they are audit-context rows. They do not replace the primary traffic winner chosen by `sm_utm_final_source_priority`.
</Info>

<Card title="Order Attribution Audit Reference" icon="table" href="/data-activation/data-tables/sm_transformed_v2/fct_order_attribution_audit">
