...
<AccordionGroup>
  ...
  <Accordion title="Order Age Limits" icon="calendar">
    ...
    - Check with SourceMedium support about attribution source hierarchy priority for your account
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="What's the priority if multiple sources have UTMs?">
    SourceMedium uses an [attribution source hierarchy](/data-transformations/attribution-source-hierarchy) that prioritizes data closest to the transaction. Contact SourceMedium support for your specific configuration.
  </Accordion>

  <Accordion title="Can I backfill orders that already have attribution in SourceMedium?">
    Yes, but the attribution source hierarchy determines which source wins. Backfilled `customAttributes` data may or may not override existing attribution depending on your configuration.
  </Accordion>
