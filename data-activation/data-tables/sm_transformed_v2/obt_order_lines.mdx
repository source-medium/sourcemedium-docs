  ...
  - name: obt_order_lines
    ...
    columns:
      ...
      - name: is_order_line_subscription
        description: >
          Boolean convenience field for whether the order line is classified as subscription. Use this instead of string matching when you want a stable filter for subscription lines.
      ...
      - name: order_line_type
        description: >
          Line-level order classification, such as subscription or one-time. For Shopify, SourceMedium prioritizes direct subscription-platform mappings first, then explicit line-item metadata, then limited legacy fallback signals such as order tags or app/source markers. Use this field for mixed-cart and product-level subscription analysis.
      ...
      - name: subscription_id
        description: >
          Subscription identifier for the line when a reliable one is available. For Shopify, placeholder values known to be bad subscription signals can be normalized away before this field is populated.

      - name: subscription_order_index
        description: >
          Ordered index identifying the sequential position of a subscription line within a customer's subscription history when a reliable subscription identifier or direct subscription-platform mapping is available.

      - name: subscription_order_sequence
        description: >
          Line-level subscription lifecycle classification. For subscription lines, this reflects first-vs-recurring subscription behavior when a reliable subscription index is available. For non-subscription lines, the sequence should resolve to the non-subscription value. SourceMedium keeps this field aligned with is_order_line_subscription so line-level sequence does not contradict the line-level subscription classification.
