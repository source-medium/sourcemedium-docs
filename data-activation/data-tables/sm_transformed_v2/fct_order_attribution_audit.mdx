---
title: 'fct_order_attribution_audit'
description: 'Row-level attribution audit table for tracing how SourceMedium evaluated each order''s attribution evidence.'
icon: "table"
---
```yaml
version: 2
...
models:
  - name: fct_order_attribution_audit
    ...
    columns:
      ...
      - name: evidence_type
        description: >
          High-level class of evidence on the row. Current values are traffic_source_candidate, zero_party_candidate, and order_discount_code. Note that traffic_source_candidate includes both primary traffic candidates and MTA audit-context rows.
      ...
      - name: sm_utm_source_medium
        description: >
          Combined canonical source / medium pair used by SourceMedium channel mapping and attribution debugging.
...
`fct_order_attribution_audit` is the customer-facing row-level table for understanding why a specific order was attributed the way it was. It keeps the winning traffic source and the supporting evidence rows side by side so you can debug source hierarchy, zero-party answers, and order discount context without dropping into raw staging models.
...
<Note>
`mta_first_touch` and `mta_last_touch` use `evidence_type = traffic_source_candidate`, but they are audit-context rows. They always have `sm_utm_final_source_priority = NULL`, and an order can have at most one row of each type when that order appears in MTA journey data.
</Note>
...
<Tip>
`mta_first_touch` and `mta_last_touch` rows are kept in the audit table as traffic context, but they do not compete for the primary traffic winner. They always have `sm_utm_final_source_priority = NULL`, and an order will have at most one of each when the order appears in MTA journey data.
</Tip>
...
- `sm_order_attribution_audit_key`: Stable key for the evidence row.
...
FROM `your_project.sm_transformed_v2.fct_order_attribution_audit`
...
<CardGroup cols={2}>
  <Card title="Attribution Source Hierarchy" icon="layer-group" href="/data-transformations/attribution-source-hierarchy">
    Learn how SourceMedium orders the traffic candidates that feed this audit table.
  </Card>
