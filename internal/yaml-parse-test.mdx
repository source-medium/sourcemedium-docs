...
models:
  - name: rpt_ad_performance_daily
    description: >
      ...
      revenue at the creative- and date-level granularity. This report (RPT) table is optimized for a BI tool's consumption.
    ...
    columns:
      ...
      - name: ad_campaign_tactic
        description: >
          The tactic of the campaign that is associated with the ad, such as "prospecting" or "retargeting" (the campaign tactic will vary by advertising platform).
      
      - name: ad_spend_retention
        description: >
          The amount of ad spend that is attributed to retention, which is defined by having a campaign tactic of "retention".
      
      - name: ad_spend_conversion
        description: >
          The amount of ad spend that is attributed to conversion, which is defined by having a campaign tactic that is not "retention".
```

bottom markdown text test